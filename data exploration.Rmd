...
#    at each month t we hold 1/N in each currency for that month’s excess return
n_curr <- returns_long %>% 
  distinct(Cur) %>% nrow()

bench_1n <- returns_long %>% 
  group_by(YearMonth) %>% 
  summarise(
...
## 3) forward‐signal portfolio
bench_forw <- returns_long %>% 
  group_by(YearMonth) %>% 
  mutate(
    sign_prem = sign(prem)
  ) %>% 
...
bench <- bind_cols(
  bench_1n,
  bench_forw = bench_forw$ForwardSignal) %>%
  inner_join(a_portfolio %>% rename(YearMonth = Date), by = "YearMonth") %>%
  mutate(
    OneN_CumulativeReturn = cumprod(1 + OneOverN) - 1,
    Forward_CumulativeReturn = cumprod(1 + bench_forw) - 1,
  )
...
bench %>% 
  ...
  ggplot(aes(YearMonth, CumulativeReturn, color = Strategy)) +
    ...
    scale_colour_viridis_d(
     ...
     #scale_color_manual(
      #   labels = c("Alpha-Based (EWMA)","Forward Signal", "1/N", "Alpha-Based", "Alpha-Based (SMA)"), 
