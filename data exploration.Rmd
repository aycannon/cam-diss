...
# Forecast data for ARIMA testing (in altrernative R file titled "ARIMA testing.R")
...
estimate_a2 <- function(currencies, wide_data, p = 2) {
  ...
  for (cur in currencies) {
    ...
    data_cur <- wide_data %>%
      transmute(
        forecast_error = .data[[s_t1]] - .data[[f_col]],     
...
ggplot(smooth_alpha, aes(x = Date)) +
...
```{r smoothed alpha portfolios NEW}
...
```{r Re-doing graph for all alpha strats}
...
```{r Plotting the smoothed strats}
a_ewma_portfolios %>%
  pivot_longer(
...
```{r Rolling alphas graphs animated}
...
### benchmark startegies
```{r benchmark 1/n}

## 1) Gather out into long format so we can loop over currencies generically
returns_long <- wide_month %>% 
  ...
  pivot_longer(
    cols = -YearMonth,
    names_to = c(".value","Currency"),
    names_sep = "_"
  ) %>%
    pivot_longer(
     cols = c(-YearMonth, -Currency),
     names_to = "Cur",
     values_to = "value"
    ) %>%
    pivot_wider(
...
## 2) 1/N equal‐weight portfolio

returns_filtered <- returns_long %>%
    mutate(Date = as.Date(YearMonth)) %>%
  filter(Date >= as.Date("2005-01-01"))

# 1) Number of currencies (if still needed)
n_curr <- returns_filtered %>% 
  distinct(Cur) %>% 
  nrow()

# 2) 1/N benchmark strategy
bench_1n <- returns_filtered %>% 
  group_by(YearMonth) %>% 
  summarise(
    OneOverN = mean(excess, na.rm = TRUE),
    .groups = "drop"
  )

# 3) Forward signal benchmark strategy
bench_forw <- returns_filtered %>% 
  group_by(YearMonth) %>%
  mutate(
    sign_prem = sign(prem)
  ) %>% 
  summarise(
    ForwardSignal = mean(sign_prem * excess, na.rm = TRUE),
    .groups = "drop"
  )


## 4) combine and compute cumulative returns
bench_both <- bench_1n %>%
  rename(OneN = OneOverN) %>%
  mutate(Forward = bench_forw$ForwardSignal) %>%
  mutate(Date = as.Date(YearMonth)) %>%
  select(Date, OneN, Forward) %>%
  mutate(
    Cume_OneN = cumprod(1 + OneN) - 1,
    Cume_Forward = cumprod(1 + Forward) - 1
  ) %>%
  pivot_longer(
    cols = starts_with("Cume_"),
    names_to = "Strategy",
    names_prefix = "Cume_",
    values_to = "CumulativeReturn"
  ) %>%
...
alpha_all <- combined_cume %>%
  select(Date, CumulativeReturn, Source, Strategy_1 = Strategy)

final_cume_all <- bind_rows(alpha_all, bench_both) %>%
  mutate(
    Strategy_1 = case_when(
      Strategy_1 == "OneN"    ~ "1/N",
      Strategy_1 == "Forward" ~ "Forward",
      TRUE                    ~ Strategy_1
    )
  )

# plotting the benchmark policies
# Main plot
ggplot(final_cume_all, aes(x = Date, y = CumulativeReturn, 
                           color = Source, linetype = Strategy_1)) + # i don't like tyhe number of line types - we should look to fix this murky graph
  geom_line() +
  theme_minimal() +
  labs(
    title = "Cumulative Returns of Alpha-Based and Benchmark Strategies",
    x = "Date",
    y = "Cumulative Return"
  ) +
  theme(
